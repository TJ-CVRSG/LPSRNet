...
import models.backbone.micronet.activation as activation
import models.backbone.micronet.micronet_configs as microcfg
...
class MicroNet(nn.Module):
    def __init__(self, cfg, input_size=224, num_classes=1000, teacher=False):
        ...
        self.f1_layer = int(cfg.feature_layers[0])
        self.f2_layer = int(cfg.feature_layers[1])
        self.f3_layer = int(cfg.feature_layers[2])

        self._initialize_weights()
           
    def forward(self, x):
        f1 = self.features[:self.f1_layer](x)               # 48x48
        f2 = self.features[self.f1_layer:self.f2_layer](f1) # 24x24
        f3 = self.features[self.f2_layer:self.f3_layer](f2) # 12x12
        f4 = self.features[self.f3_layer:](f3)              #  6x6
...
def create_micronet(cfg, input_size=192):
    """
    Constructs a MicroNet model
    """
    return MicroNet(cfg, input_size=input_size)