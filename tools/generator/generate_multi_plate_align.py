...
class MultiPlateGeneratorAlign:
    def __init__(self, adr_plate_model, adr_font):
        ...
        for font_filename in font_filenames:
            ...
            self.font_imgs[os.path.basename(
                font_filename).split('.')[0]] = font_img
    ...
    def generate_plate(self, enhance=False):
        ...
        img_plate_model = cv2.imread(os.path.join(
            self.adr_plate_model, '{}_{}.PNG'.format(bg_color, height)))
        img_plate_model = cv2.resize(
            img_plate_model, (440 if len(plate_number) == 7 else 480, height))
        ...
        for i in range(len(plate_number)):
            ...
            if enhance:
                ...
                if np.random.random(1) > 0.5:
                    ...
                    font_img = np.copy(cv2.dilate(
                        font_img, kernel, iterations=1))
    ...
    def generate_plate_special(self, plate_number, bg_color, is_double, enhance=False):
        ...
        img_plate_model = np.zeros_like(img_plate_model) + 255

        for i in range(len(plate_number)):
            if plate_number[i] == ' ':
                continue
            if len(plate_number) == 8:
                if plate_number[i] == '学' or plate_number[i] == '警' or plate_number[i] == '领' in plate_number:
        ...
        img_plate_model = cv2.cvtColor(img_plate_model, cv2.COLOR_BGR2GRAY)
...
if __name__ == '__main__':
    ...
    mkdir(args.save_adr)
    generator = MultiPlateGeneratorAlign('plate_model', 'font_model')
